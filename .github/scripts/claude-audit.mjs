...
const SYSTEM = `You are reviewing a diff from a PUBLIC repo whose scheduled workflow runs build-cache.js to rebuild cache.json from an external API. Watch for leaked credentials (repo is public), injection into the build script, and workflow supply-chain slips.
